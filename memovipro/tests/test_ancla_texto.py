...
def test_mensaje_de_borrado_no_finge_un_rango():
    ...
    assert confirmar_borrado([4]) == "¿Eliminar el paso 5?"
    assert "del 3 al 5" in confirmar_borrado([2, 3, 4])      # contiguos
    suelta = confirmar_borrado([2, 6, 39])
    assert "3, 7, 40" in suelta and "al 40" not in suelta
    assert "y 5 más" in confirmar_borrado(list(range(0, 30, 2)))
    assert confirmar_borrado([]) == "No hay pasos seleccionados."
    # El orden de entrada no importa
    assert confirmar_borrado([4, 2, 3]) == confirmar_borrado([2, 3, 4])
